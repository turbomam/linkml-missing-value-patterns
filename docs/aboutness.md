...
| [p07_restricted_access](https://github.com/turbomam/linkml-missing-value-patterns/blob/main/rdf-examples/p07_restricted_access.ttl) | `dnaDatum1`; `status7` | both `napoleon` and `descendant1`; the status is about `dnaDatum1` | unclear: two subjects, so it does not say whose result is withheld |
...
No Napoleon schema has a slot that says what a record is about. `HeightRecord` implies Napoleon only
through its class name and description, so aboutness lives in the schema, not in the data.

| Record | What it is about | Aboutness |
|---|---|---|
| [strict/valid_measured](https://github.com/turbomam/linkml-missing-value-patterns/blob/main/napoleon/data/strict/valid_measured.yaml) and the other measured records | Napoleon's height, by class name only | implicit |
...
| [not_applicable/valid_hair_lock](https://github.com/turbomam/linkml-missing-value-patterns/blob/main/napoleon/data/not_applicable/valid_hair_lock.yaml) | a hair-lock relic | clear by class name only |

## What this suggests

- Only the side-report pattern states aboutness in the data. Every other LinkML record depends on the
  class name, and two records contradict theirs. An explicit `about` slot, ranged over identifiers
...
- In RDF, aboutness is clear whenever a data item exists. It is absent when nothing was produced
  (p01, p05), shifted to another entity in p10, and split between two subjects in p07.
